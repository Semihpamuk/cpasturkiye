...
export async function POST(req: Request) {
  try {
    ...
    const order = {
      ...
      isAgency: pending?.isAgency ?? false,
      storeCount: pending?.storeCount ?? 1,
      billing: (pending?.billing ?? "monthly") as "monthly" | "yearly",
      installment: installmentKey,
      discountCode: pending?.discountCode ?? null,
      subscriptionNet: pending?.subscriptionNet ?? 0,
      includeSetup: pending?.includeSetup ?? false,
    ...
    await sendOrderConfirmation({
      ...
      isAgency: order.isAgency,
      includeSetup: order.includeSetup,
